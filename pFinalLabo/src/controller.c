...
int controller_loadFromText(char* path , LinkedList* pArrayListServicios)
{
	...
	{
		pFile = fopen (path, "r");//abro archivo indicado y en forma de lectura
		if (pFile == NULL)
		{
...
int controller_sortPassenger(LinkedList* pArrayListServicios)
{
	...
	if(pArrayListServicios != NULL) //printf("\n previo a validar");
	{

		retorno = 1;
		limite = ll_len(pArrayListServicios);//obtengo tamano de LL

		do
		{
			flagSwap = 0;

			for(i=0;i<limite-1;i++)//limite menos 1
			{
				for(int j=i+1;j<limite;j++)//lista completa
				{
					pAuxServicio = ll_get(pArrayListServicios,i);//obtengo ambos punteros
					pAuxSigServicio = ll_get(pArrayListServicios,j);

					if(pAuxServicio != NULL && pAuxSigServicio  != NULL)//verifico que ambos sean diferetes de null
					{

						Servicios_getDescripcion(pAuxServicio,bufferDescripcion);//obtengo la descripcion de cada uno
						...
						if(strncmp(bufferDescripcion,bufferSigDescripcion,LENDESCRIPCION) > 0)//pregunto si el sigueinte es menor
						{
							ll_set(pArrayListServicios, i, pAuxSigServicio);//intercambio nodos
							ll_set(pArrayListServicios, j, pAuxServicio);
							flagSwap=1;//aviso que hice un cambio
						}
			...
			limite--;//achivo el limite porque el ultimo siempre va a ser el mas grande
		}while(flagSwap);
	}
    return retorno;
}
...
int controller_Map (LinkedList* listaServicios)
{
	...
	{
		//llamado a map
		ll_map(listaServicios, calcular); //completa un campo del elemento paso linkedlist y funcion
