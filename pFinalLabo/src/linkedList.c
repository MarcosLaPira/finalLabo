...
LinkedList* ll_newLinkedList(void)
{
    ...
    this = (LinkedList*)malloc(sizeof(LinkedList));//pide lugar en memoria

    if (this != NULL)
    {
		this->pFirstNode = NULL;//inicializa
		this->size = 0;
    }
    return this;
}
...
int ll_len(LinkedList* this)
{
    ...
    if (this!= NULL)
    {
    	returnAux = this->size;//valor del campo int zise en LL
    }
    return returnAux;
}
...
static Node* getNode(LinkedList* this, int nodeIndex)// 1ro
{
	Node* pNode = NULL;
	if(this != NULL && nodeIndex >= 0 && nodeIndex < ll_len(this))//valida ll y que el indice sea >0 0 pq linkedlist comienz en -1
	{
		pNode = this ->pFirstNode; // en el caso de que sea 0 pNode esta en el primer lugar

		for (int i = 0;i<nodeIndex; i++) // menor al indice a buscar, asi no recorro tod
		{
...
void* ll_get(LinkedList* this, int index)
{
    ...
    {
    	pNode = getNode(this, index);//paso linkedlist y nuemro de nodo
    	if (pNode != NULL)
    	{
...
int ll_set(LinkedList* this, int index,void* pElement) // BORRAR un Nodo PISA EL NODO
{
    ...
    if (this != NULL && index < ll_len(this) && index >= 0)
    {
    	pNode = getNode(this, index);//obtengo nodo
    	if(pNode != NULL)
    	{
    ...
    if (this != NULL && index < ll_len(this) && index >= 0)
       {
    		...
    		auxNode  = getNode(this, index);   // obtengo nodo a  eliminar
			if (index == 0)//en caso de ser el primer nodo
			{
				this ->pFirstNode = auxNode ->pNextNode;//gaurdo en el primer nodo la direcicion del siguiente para no perder referencia
			}
			else//si no es el primero
			{
				...
				if (pNodePrevio != NULL)
					{
						pNodePrevio ->pNextNode = auxNode ->pNextNode;//enlazo el nodo previo al siguiente perdiendo la diferencia del que queda entre medio
					}
			}
			free(auxNode);
			this->size--;//decremento tamano de LL
			returnAux = 0;
       	}
    return returnAux;
}
...
int ll_push(LinkedList* this, int index, void* pElement) //desplaza el nodo  e inserta un nuevo nodo para poner el un nuevo nuevo elemento
{
    int returnAux = -1;
...
//pop: devuelve el nodo eliminado
...
int ll_containsAll(LinkedList* this,LinkedList* this2)// compara si todos los elementos de la LL this2 estan en LL this
{
    ...
    if(this != NULL && this2 != NULL)
    {
    	...
    	for(int i = 0; i<ll_len(this2);i++) // menor si o si porque lo estoy agregando
    	{
    		...
    		if(!(ll_contains(this,pElementAux))) // paso LL1 y el elemeto de LL2 y verifico si existe
    		{
    			returnAux = 0;
...
LinkedList* ll_subList(LinkedList* this,int from,int to) // copia la lista desde un rango hasta otro excluido
{
    ...
    {
    	cloneArray = ll_newLinkedList();//inicializo nueva linkedlist

    	if(cloneArray != NULL)//verifico si ll es != NULL
    	{
    		for(int i=from; i <to ;i++)  //indico rangos a copiar
    		{
    			pElementAux = ll_get(this,i);  //obtengo elemento indicado de LL verdadera
    			ll_add(cloneArray,pElementAux);//agrego en LL a copiar
    		}
    	}
    }
    return cloneArray;
...
{
	if(this != NULL && fn != NULL)// verifica que nada venga nulo
	{
		for(int i = 0; i<ll_len(this);i++)//ciclo siempre que sea menor a linledlist
		{
			fn(ll_get(this,i));//llamado a calcular, obtiene puntero
		}
	}
//	return retorno;
